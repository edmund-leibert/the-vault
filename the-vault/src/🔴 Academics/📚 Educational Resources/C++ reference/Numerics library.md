...
**Flashcard 1**:
Q: What are the C++ mathematical functions for computing the smallest integer value not less than the given value? #anki-card-reverse 
A: `std::ceil`, `std::ceilf`, and `std::ceill`


**Flashcard 2**:
Q: What is the prototype of the `std::ceil` function in C++? #anki-card-reverse 
A: double `std::ceil(double x) noexcept;`


**Flashcard 3**:
Q: What is the prototype of the `std::ceilf` function in C++? #anki-card-reverse 
A: `float std::ceilf(float x) noexcept;`


**Flashcard 4**:
Q: What is the prototype of the `std::ceill` function in C++? #anki-card-reverse 
A: `long double std::ceill(long double x) noexcept;`


**Flashcard 5**:
Q: Which header should be included to use the `std::ceil` functions in C++? #anki-card 
A: `#include <cmath>`


**Flashcard 6**:
Q: Which C++ function should be used when dealing with float type arguments? #anki-card-reverse 
A: `std::ceilf`


**Flashcard 7**:
Q: Which C++ function should be used when dealing with long double type arguments? #anki-card-reverse 
A: `std::ceill`


**Flashcard 8**:
Q: Which C++ function should be used when dealing with double type or default arguments? #anki-card-reverse 
A: `std::ceil`


**Front:** What is an exception in C++? #anki-card-reverse 
...
**Front:** What happens when an exception is thrown in C++? #anki-card 
...
**Front:** What does the `noexcept` keyword mean in C++? #anki-card-reverse  
...
**Front:** What happens if an exception occurs within a function declared with `noexcept`? #anki-card 
